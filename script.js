function getQuerys(){
  try {
    ...
    return res
  } catch(e){window.location.href = '/v1/crashed.html#:'+Date.now()+':#e!'+e.stack}
  }
function sessionCheck(){
  try {
    ...
    if(s_done == false){
      var sessioninter = setInterval('sessionCheck();window.location.href=window.location.href.split("#")[0]+"#"+Date.now()', 3000)
      var s_done = true
    }
  } catch(e){window.location.href = '/v1/crashed.html#:'+Date.now()+':#e!'+e.stack}
}
function sessionCheckC(){
  try {
    ...
    if(sessioniter2 == undefined){
      var sessioninter2 = setInterval('sessionCheckC();window.location.href=window.location.href.split("#:")[0]+"#:"+Date.now()+":#"+window.location.href.split(":#")[1]', 3000)
    }
  } catch(e){window.location.href = '/v1/crashed.html#:'+Date.now()+':#e!'+e.stack}
}